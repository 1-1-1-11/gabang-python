...
一个基于 Python 的五子棋 AI 项目，使用 FastAPI 提供后端接口，并提供可直接打开的静态前端页面用于本地对弈和演示。
...
- 静态前端：支持棋盘大小、搜索深度、AI 先手、API 地址配置、落子记录、最近一步高亮和 AI 搜索信息展示。
...
```text
http://127.0.0.1:8765/index.html?apiBase=http://127.0.0.1:8000
```
...
如需运行浏览器端验收测试，还需要安装 Node.js 和 npm。
...
```text
frontend/index.html?apiBase=http://127.0.0.1:9000
```
...
7. 打开静态前端，完成“开始、落子、AI 回复、悔棋、结束”流程。
