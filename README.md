...
- `d37ac32` 引入的迭代加深、静态搜索和 `beta_cutoffs` 指标已有 24-B 固定局面评估基线和 24-C 跨版本指标对比；结论是关键战术选点保持稳定、部分局面搜索更深但更慢，不能直接表述为棋力收益已证明。
...
| AI 棋力收益仍需更多局面证明 | 24-C 已完成 `d37ac32` 前后固定局面对比：战术选点稳定，但非终局局面节点和耗时上升 | 更深搜索不自动等于更强棋力 | 后续若要证明收益，需要更多中盘/终局局面和对弈评测 |
...
| P1 | AI 棋力评测扩展 | 24-C 已完成首轮跨版本指标对比 | 增加更多中盘/终局局面、失败案例和对弈评测，避免只凭单批指标下结论 |
